...
7. 톰캣이 경로에 있는 프로젝트를 자동으로 배포 

<br><br>

## docker jenkins 자동 빌드 및 배포환경 구성

위 방법은 프로젝트에 변동사항이 생길 때마다 수동으로 배포를 반복해야 하는 단점이 있었습니다. 그래서 jenkins와 docker를 사용해서 한 개의 EC2 인스턴스에서 자동 빌드 및 배포 환경을 구성하게 되었습니다. 그 과정을 개인 블로그에 직접 정리하면서 진행했습니다.
...
4. 윈도우 웹브라우저에서 jenkins 관리자 페이지 접속 후 자동 빌드 및 배포 설정
